...
class PersonalityTestViewModel extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      if (_sessionId == null || _sessionId!.isEmpty) {
        _sessionId = await guestApi.createSession();
        await StorageService.saveSessionId(_sessionId!);
      }
      
      // 3. 질문 로드
      _questions = await guestApi.getQuestions();
  ...
  Future<void> submitAnswers() async {
    ...
    try {
      // 1. 답변 제출
      await guestApi.submitAnswers(_sessionId!, _answers);
      
      // 2. 결과 조회
      _result = await guestApi.getResult(_sessionId!);
